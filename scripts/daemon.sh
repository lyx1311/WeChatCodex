...
# =============================================================================
# WeChatCodex cross-platform daemon manager
# Supports: macOS (launchd) / Linux (systemd + nohup fallback)
# =============================================================================

DATA_DIR="${HOME}/.wechat-codex-bridge"
PROJECT_DIR="$(cd "$(dirname "$0")/.." && pwd)"
SERVICE_NAME="wechat-codex-bridge"
DISPLAY_NAME="WeChatCodex"
...
macos_plist_label() {
  echo "com.wechat-codex-bridge.daemon"
}
...
PLIST
  ...
  echo "Started ${DISPLAY_NAME} daemon (macOS launchd)"
}
...
macos_stop() {
  ...
  rm -f "$plist_path"
  echo "Stopped ${DISPLAY_NAME} daemon (macOS launchd)"
}

macos_status() {
  if macos_is_loaded; then
    local pid=$(pgrep -f "dist/src/main.js start" 2>/dev/null | head -1)
    if [ -n "$pid" ]; then
...
[Unit]
Description=WeChatCodex
Documentation=https://github.com/DrDavidDa/WeChatCodex
After=network.target

[Service]
Type=simple
ExecStart=${node_bin} ${PROJECT_DIR}/dist/src/main.js start
...
linux_direct_start() {
  ...
  echo "Starting ${DISPLAY_NAME} daemon (direct mode)..."
  nohup "$node_bin" "${PROJECT_DIR}/dist/src/main.js" start \
...
linux_start() {
  if linux_systemd_available; then
    ...
    echo "Started ${DISPLAY_NAME} daemon (Linux systemd)"
  else
...
linux_stop() {
  if linux_systemd_available && systemctl --user cat "${SERVICE_NAME}" &>/dev/null; then
    ...
    echo "Stopped ${DISPLAY_NAME} daemon (Linux systemd)"
  else
    linux_direct_stop
  fi
}
